...
PDF = Path("raw/2022-23/monthly/FR_APr_2022.pdf")

OUTPUT = Path(
    "processed/extracted/historical_2022_april.json"
)

START_PAGE = 83
END_PAGE = 179

EXPECTED_PROJECTS = 1559
...
def parse_numeric_fields(words, row_y, next_code_y=None):
    ...
    for w in project_words:
        if 495 <= w["x0"] < 525:
            ...
            if m:
                delays.append((w["y0"], w["x0"], int(m.group(1)), m.group(2)))
...
def parse_identity(words, anchor, previous_anchor=None, next_anchor=None):
    ...
    if previous_anchor and previous_anchor["page"] == page_no:
        ...
        prev_y = 210 if page_no == START_PAGE else 140
    ...
    match = re.search(r"\[[A-Za-z0-9]+\]([^,]*)(?:,(.*))?", code_text)
    if match:
...
def extract_project(page_data, anchor, previous_anchor, next_anchor):
    words = page_data["words"]
    identity = parse_identity(words, anchor, previous_anchor, next_anchor)
...
def main():
    print("=" * 100)
    print("2022-23 APRIL HISTORICAL EXTRACTION")
    print("=" * 100)
